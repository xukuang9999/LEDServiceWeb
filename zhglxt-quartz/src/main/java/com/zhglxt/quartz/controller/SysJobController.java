...
public class SysJobController extends BaseController
{
    ...
    @Log(title = "Schedule Job", businessType = BusinessType.EXPORT)
    ...
    public AjaxResult export(SysJob job)
    {
        ...
        return util.exportExcel(list, "Schedule Job");
    }

    @Log(title = "Schedule Job", businessType = BusinessType.DELETE)
     ...
     * 任务调度状态修改
     */
    @Log(title = "Schedule Job", businessType = BusinessType.UPDATE)
    ...
    public AjaxResult run(SysJob job) throws SchedulerException
    {
        ...
        return result ? success() : error("The job don't exist or has expired");
    }
     ...
     * 新增保存调度
     */
    @Log(title = "Schedule Job", businessType = BusinessType.INSERT)
    @RequiresPermissions("monitor:job:add")
    @PostMapping("/add")
    @ResponseBody
    ...
    public AjaxResult editSave(@Validated SysJob job) throws SchedulerException, TaskException
    {
        if (!CronUtils.isValid(job.getCronExpression()))
        {
            return error("Edit job '" + job.getJobName() + "'失败，Cron表达式不正确");
        }
        else if (StringUtils.containsIgnoreCase(job.getInvokeTarget(), Constants.LOOKUP_RMI))
        {
            return error("Edit job '" + job.getJobName() + "'失败，目标字符串不允许'rmi'调用");
        }
        else if (StringUtils.containsAnyIgnoreCase(job.getInvokeTarget(), new String[] { Constants.LOOKUP_LDAP, Constants.LOOKUP_LDAPS }))
        {
            return error("Edit job '" + job.getJobName() + "'失败，目标字符串不允许'ldap'调用");
        }
        else if (StringUtils.containsAnyIgnoreCase(job.getInvokeTarget(), new String[] { Constants.HTTP, Constants.HTTPS }))
        {
            return error("Edit job '" + job.getJobName() + "'失败，目标字符串不允许'http(s)'调用");
        }
        else if (StringUtils.containsAnyIgnoreCase(job.getInvokeTarget(), Constants.JOB_ERROR_STR))
        {
            return error("Edit job '" + job.getJobName() + "'失败，目标字符串存在违规");
        }
        else if (!ScheduleUtils.whiteList(job.getInvokeTarget()))
        {
            return error("Edit job '" + job.getJobName() + "'失败，目标字符串不在白名单内");
        }
    ...
    public AjaxResult queryCronExpression(@RequestParam(value = "cronExpression", required = false) String cronExpression)
    {
        ...
        {
            return error("The cron expression don't exist");
        }
    }
}
