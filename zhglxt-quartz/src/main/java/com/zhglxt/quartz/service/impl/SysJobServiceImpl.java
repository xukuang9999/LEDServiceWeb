...
public class SysJobServiceImpl implements ISysJobService
{
    ...
    public void init() throws SchedulerException, TaskException
    {
        try {
            ...
            // 检查是否是 Quartz 表不存在的错误
            if (e.getMessage() != null && e.getMessage().contains("doesn't exist")) {
                System.err.println("警告: Quartz 数据库表不存在，请执行以下步骤解决：");
                System.err.println("1. 执行 sql/fix_quartz_mysql8_compatibility.sql 脚本");
                System.err.println("2. 或者执行 sql/quartz_tables.sql 脚本");
                System.err.println("3. 确保数据库中存在所有 qrtz_ 开头的表");
                System.err.println("错误详情: " + e.getMessage());
                
                // 记录日志但不阻止应用启动
                System.err.println("定时任务模块初始化失败，但应用将继续启动...");
                return;
            }
            // 其他异常继续抛出
            throw e;
