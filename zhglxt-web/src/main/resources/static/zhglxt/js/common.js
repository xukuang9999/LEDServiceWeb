...
function checkpwd(chrtype, password) {
    if (chrtype == 1) {
        if (!$.common.numValid(password)) {
            $.modal.alertWarning("The password only supports 0-9 numbers");
            return false;
        }
    } else if (chrtype == 2) {
        if (!$.common.enValid(password)) {
            $.modal.alertWarning("The password only supports a-z and A-Z letters");
            return false;
        }
    } else if (chrtype == 3) {
        if (!$.common.enNumValid(password)) {
            $.modal.alertWarning("The password must simultaneously contain both numbers and letters");
            return false;
        }
    } else if (chrtype == 4) {
        if (!$.common.charValid(password)) {
            $.modal.alertWarning("The password must simultaneously contain letters, numbers and characters<font color='red'>~!@#$%^&*()-=_+</font>");
            return false;
        }
    }
    return true;
...
$.ajaxSetup({
    ...
    complete: function(XMLHttpRequest, textStatus) {
        if (textStatus == 'timeout') {
            $.modal.alertWarning("The server is timeout, please wait and try again!");
            $.modal.enable();
            $.modal.closeLoading();
        } else if (textStatus == "parsererror" || textStatus == "error") {
            $.modal.alertWarning("The server is error, please contact with administrator!");
            $.modal.enable();
            $.modal.closeLoading();
        }
    }
